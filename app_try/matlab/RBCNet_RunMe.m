...
opath='./Data/';
folders = dir(opath);
Output_path='../tmp/Segmentation_Output/';
...
val_data = imread(image_path); 
val_data=imresize(val_data,0.3);
val_dataf=val_data;
ss = size(val_data);
val_data(:,:,4)=ones(ss(1),ss(2));
...
for k =1: numberOfBlobs           % Loop through all blobs.
    ...
    [bboxes,scores] = detect(detector,im2_connected_comp); % detect is a matlab custom function
    bboxes=bboxes/0.3;
    n=(size(bboxes,1));
...
allrect_Path =strcat(Output_path,'_Bounding Boxes_');
allscores_path =strcat(Output_path,'_Scores_');
vis_path =strcat(Output_path,'_Visualization_');
...
rectpath=strcat(allrect_Path,'.mat');
save(rectpath,'allrect');
scorepath=strcat(allscores_path,'.mat');
save(scorepath,'allscores');
wpathFRCNN=strcat(vis_path,'.png');
